...
if not os.path.exists(MODEL_FILE_PATH):
    print("Modelo 'knn_classifier_model.pkl' não encontrado.")
    print("Execute antes o script de treinamento do KNN para gerar e treinar o modelo.")
    sys.exit(1)
...
def predict_favorite_recipes_knn(user_liked_ingredients):
    if model is None or pca is None or scaler is None or label_encoder is None:
        raise ValueError("Um ou mais componentes do modelo (KNN, PCA, Scaler, LabelEncoder) não foram carregados. Verifique o caminho do arquivo do modelo.")
    ...
    probs = model.predict_proba(user_data_pca)[0]
